...
class plot_CM():
    ...
    def prepr_corr(self):
        ...
        d_ToRename = {
            ...
            'Sjogren_Syndrome': "Sjögren's syndrome",
    ...
    def plot_CM_heatmap_v3(self):
        ...
        g = sns.clustermap(
            ...
            cbar_kws={
                ...
                # "label": "$r$\n(calculated using summary-imputed association z-scores across 298 classical HLA alleles for 33 UKB diseases)"
                "label": "$r$\n(calculated using summary-imputed association z-scores across 298 classical HLA alleles)"
            },
            # cbar_pos=(0.35, 0.05, 0.4, 0.02),  # (left, bottom, width, height),
            cbar_pos=(0.305, 0.05, 0.4, 0.02),  # (left, bottom, width, height),
...
if __name__ == "__main__":
    ...
    mpl.rcParams['mathtext.it'] = 'Arial:italic'    
        # 이 두 setting은 아래 "r" 이 single charactter arial-italic으로 넣을 때 필요함. (2026.05.09.0647PM. 토요일 아이디헤어 주변 카페에서.)

    ## ── Font ──────────────────────────────────────────────────────────────────
    _DIR_ARIAL = "/data02/wschoi/_hCAVIAR_v2/Arial_fonts"
    for _fname in ["Arial.ttf", "Arial Bold.ttf", "Arial Italic.ttf", "Arial Bold Italic.ttf"]:
        fm.fontManager.addfont(join(_DIR_ARIAL, _fname))
    mpl.rcParams['font.family'] = fm.FontProperties(fname=join(_DIR_ARIAL, "Arial.ttf")).get_name()
    mpl.rcParams['svg.fonttype'] = 'none'

    ## ── Paths ─────────────────────────────────────────────────────────────────
    _DIR_SUM2HLA = "/data02/wschoi/_hCAVIAR_v2/SUM2HLA"
    _DIR_CM      = "/data02/wschoi/_hCAVIAR_v2/Paper_Fig_CM"

    _H5AD_UMAP = join(_DIR_CM, "adata.UKBB.T33.2field.UMAP.spread_0_3.min_dist_0.h5ad")
    _H5AD_tSNE = join(_DIR_CM, "adata.UKBB.T33.2field.tSNE.early_exagg_3.lrate_100.h5ad")

    ## ── Build 2-field adata (or load from cache) ──────────────────────────────
    sys.path.insert(0, _DIR_SUM2HLA)
    import analysis._20250928_CM as CM

    if exists(_H5AD_UMAP) and exists(_H5AD_tSNE):
        print("Loading cached 2-field adata...")
        adata_UMAP = sc.read_h5ad(_H5AD_UMAP)
        adata_tSNE = sc.read_h5ad(_H5AD_tSNE)
    else:
        print("Generating 2-field adata from scratch...")
        df_zscore_2field = pd.read_csv(
            join(_DIR_CM, "UKBB.Z_table.tr.only_HLA_2field.txt"), sep='\t', header=0, index_col=0
        )

        d_KNN = CM.gen_Anndata(_df_zscore=df_zscore_2field)
        adata_base = CM.reset_category_names(d_KNN[5])

        ## UMAP: spread=0.3, min_dist=0.0
        adata_UMAP = adata_base.copy()
        sc.tl.umap(adata_UMAP, spread=0.3, min_dist=0.0)
        adata_UMAP.write_h5ad(_H5AD_UMAP)
        print(f"Saved: {_H5AD_UMAP}")

        ## tSNE: early_exaggeration=3, learning_rate=100, perplexity=5
        adata_tSNE = adata_base.copy()
        sc.tl.tsne(adata_tSNE, perplexity=5, early_exaggeration=3, learning_rate=100)
        adata_tSNE.write_h5ad(_H5AD_tSNE)
        print(f"Saved: {_H5AD_tSNE}")

    ## ── Correlation matrix (2-field) ─────────────────────────────────────────
    df_corr = pd.read_csv(join(_DIR_CM, "UKBB.Z_table.tr.only_HLA_2field.corr"), sep='\t', header=0)

    ## ── Plot & Export ─────────────────────────────────────────────────────────
    plotter = plot_CM_v2(adata_UMAP, adata_tSNE, df_corr, _figsize=(8.5, 8.5), _dpi=100)
    _, g = plotter.run_v3()

    _OUT = join(_DIR_SUM2HLA, "Fig.CM.20260509.pdf")
    g.savefig(_OUT, bbox_inches="tight")
    g.savefig(_OUT.replace("pdf", "svg"), format="svg", dpi=300, bbox_inches="tight")
    g.savefig(_OUT.replace("pdf", "png"), format="png", dpi=300, bbox_inches="tight", facecolor='white')
