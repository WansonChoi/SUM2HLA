...
class plot_HLA_manhattan_PP_v2():
    ...
    def prepr_PP(self):
        ...
        self.df_ToPlot['APP'] = self.df_ToPlot['APP'].fillna(0.0)
    ...
    def plot_HLA_manhattan_PP(self, _ax, _color_top='#d62728'):
        ...
        df = pd.DataFrame({"y": df_ToPlot['APP'].tolist()}, index=idx)
        print(df)
        ...
        if np.any(CredibleSet_99_mask):
            ...
            _ax.scatter(x[CredibleSet_99_mask], y[CredibleSet_99_mask],
                        s=16, color=_color_top, marker='D', edgecolors='none', zorder=2, alpha=0.9, label='APP ≥ 0.10',
        ...
        rules = [
            {"mask": lambda v: v >= 0.30, "label": "APP ≥ 0.30", "color": "#e41a1c", "size": 52},
            {"mask": lambda v: v >= 0.10, "label": "0.10 ≤ APP < 0.30", "color": "#377eb8", "size": 30},
            {"mask": lambda v: v >= 0.01, "label": "0.01 ≤ APP < 0.10", "color": "#4daf4a", "size": 22},
        ]
...
class plot_Figure_FirstEverHLAfm_4diseases():
    ...
    def run(self):
        ...
        _xt, _yt = x_top_mask[0] - 1300, y_top_mask[0] - 0.3
        _gap_pt = 13  # total offset in points from italic top to regular top; DP \n spacing ~16pt
        _ax_temp.annotate("HLA-DQB1*03:02",
            xy=(x_top_mask[0], y_top_mask[0]), xycoords="data",
            xytext=(_xt, _yt), textcoords="data",
            ha="center", va="top",
            fontproperties=self.arial_italic, fontsize=13,
            arrowprops=dict(arrowstyle="-|>", color="#636363", lw=1.0, shrinkA=2, shrinkB=8),
        )
        _ax_temp.annotate(f"(APP={_PP_toPrint})",
            xy=(_xt, _yt), xycoords="data",
            xytext=(0, -_gap_pt), textcoords="offset points",
        ...
        # _marker_top = "HLA-DRB1*07:01,\nHLA-DRβ1 positions 11\n(Gly), 13 (Tyr), 14 (Lys),\n25 (Gln), 30 (Leu), and 74 (Gln)\n(7 joint top; Total APP=0.56)"
        # _marker_top = "HLA-DRB1*07:01,\nSix amino acid positions in HLA-DRB1\n(7 joint top; Total APP=0.56)"
        _marker_top = "HLA-DRB1*07:01 and\nSix HLA-DRβ1 positions\n(7 joint top; Total APP=0.56)"
        _fontsize_text = 10.5
        ...
        _xt, _yt = x_top_mask[0] - 1050, y_top_mask[0] - 0.02
        _gap_pt  = 12  # italic → line2 offset (pts)
        _gap_pt2 = 13  # line2 → line3 offset (pts)
        _ax_temp.annotate("HLA-DRB1*07:01",
            xy=(x_top_mask[0], y_top_mask[0]), xycoords="data",
            xytext=(_xt, _yt), textcoords="data",
            ha="center", va="top",
            fontproperties=self.arial_italic, fontsize=_fontsize_text,
            arrowprops=dict(arrowstyle="-|>", color="#636363", lw=1.0, shrinkA=2, shrinkB=5),
        )
        _ax_temp.annotate("and Six HLA-DRβ1 positions",
            xy=(_xt, _yt), xycoords="data",
            xytext=(0, -_gap_pt), textcoords="offset points",
            ha="center", va="top", fontsize=_fontsize_text,
        )
        _ax_temp.annotate("(7 joint top; Total APP=0.56)",
            xy=(_xt, _yt), xycoords="data",
            xytext=(0, -(_gap_pt + _gap_pt2)), textcoords="offset points",
            ha="center", va="top", fontsize=_fontsize_text,
        )
        ...
        _xt, _yt = x_top_mask[0] - 1300, y_top_mask[0] - 0.3
        _gap_pt = 13  # total offset in points from italic top to regular top; DP \n spacing ~16pt
        _ax_temp.annotate("HLA-DQB1*06:02",
            xy=(x_top_mask[0], y_top_mask[0]), xycoords="data",
            xytext=(_xt, _yt), textcoords="data",
            ha="center", va="top",
            fontproperties=self.arial_italic, fontsize=13,
        ...
        _PP_toPrint = 1.0
        _marker_top = f"HLA-DRβ1 position 67L\n(APP={_PP_toPrint})"
        _color = _color_DP
        ...
        _ax_temp.annotate(
            _marker_top,
            xy=(x_top_mask[0], y_top_mask[0]), xycoords="data",
            xytext=(x_top_mask[0] - 1060, y_top_mask[0] - 0.4), textcoords="data",
            ha="center", va="center",
            arrowprops=dict(
                arrowstyle="-|>", 
                color="#636363", 
...
if __name__ == '__main__':
    ...
    d_PP = {
        "AK":              "/data02/wschoi/_hCAVIAR_v2/20251015_MVP_whole_EUR_run/MVP.GCST90476204.Actinic_keratosis.AA+HLA.PP",
        "CLE":             "/data02/wschoi/_hCAVIAR_v2/20251015_MVP_whole_EUR_run/MVP.GCST90476182.Cutaneous_lupus_erythematosus.AA+HLA.PP",
        "Mycoses":         "/data02/wschoi/_hCAVIAR_v2/20251015_MVP_whole_EUR_run/MVP.GCST90477160.Mycoses.AA+HLA.PP",
        "Dermatophytosis": "/data02/wschoi/_hCAVIAR_v2/20251015_MVP_whole_EUR_run/MVP.GCST90475561.Dermatophytosis.AA+HLA.PP",
    }
    d_df_PP = {k: pd.read_csv(v, sep='\t', header=0) for k, v in d_PP.items()}

    df_T1DGC_bim_SNP_HLA = pd.read_csv(
        "/data02/wschoi/_ClusterPhes_v4/LD_from_HLA_reference_panel/REF_T1DGC.hg19.SNP+HLA.bim",
        sep='\t', header=None, names=['CHR', 'SNP', 'GD', 'BP', 'A1', 'A2']
    )

    plotter = plot_Figure_FirstEverHLAfm_4diseases(
        d_df_PP['AK'], d_df_PP['CLE'], d_df_PP['Mycoses'], d_df_PP['Dermatophytosis'],
        df_T1DGC_bim_SNP_HLA,
        _figsize=(8, 5.5),
        _w_h_space_constrained_layout=(0.03, 0.025)
    )
    fig, _ = plotter.run()

    output_path = "/data02/wschoi/_hCAVIAR_v2/SUM2HLA/Fig.the_4_diseases.v3.pdf"
